...
import { fetchCandidateHistoryRequest } from "../../../../../store/reducers/candidateHistorySlice";
...
const HistoryTab = () => {
  ...
  const { history, loading } = useSelector(
    (state: RootState) => state.candidateHistory
  );
  const fetchPaginatedHistory = useCallback(
    (id: number) => {
      dispatch(fetchCandidateHistoryRequest({ id }));
    },
    [dispatch]
  );
  ...
  useEffect(() => {
    if (selectedCandidate?.candidateId) {
      fetchPaginatedHistory(selectedCandidate?.candidateId);
    }
  }, [dispatch, fetchPaginatedHistory, selectedCandidate?.candidateId]);
  ...
  const customizedContent = (item: CandidateHistoryInterface) => {
  ...
  return (
    <div className="w-full px-5 pt-5 bg-white rounded-lg">
      ...
      {(loading || candidateLoading) && <AtsLoader />}
