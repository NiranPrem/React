...
} from "../../../../../store/reducers/candidateResumeSlice";
...
const ResumeTab = () => {
  ...
  const { resume, loading, totalCount, updateSuccess } = useSelector(
    (state: RootState) => state.candidateResumes
  );
  ...
  const fetchPaginatedResume = useCallback(
    (firstIndex: number, rowCount: number) => {
      ...
      dispatch(
        fetchCandidateResumeRequest({ pageNumber, pageSize, candidateId })
      );
    },
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      dispatch(updateCandidateResumeRequest(payload));
      e.target.value = "";
    } catch (error) {
