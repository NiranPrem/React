...
import { fetchJobOpeningHistoryRequest } from "../../../../../store/reducers/jobOpeningHistorySlice";
...
const HistoryTab = () => {
  ...
  const { history, loading } = useSelector(
    (state: RootState) => state.jobOpeningHistory
  );
  const fetchPaginatedHistory = useCallback(
    (id: number) => {
      dispatch(fetchJobOpeningHistoryRequest({ id }));
    },
    [dispatch]
  );
  ...
  useEffect(() => {
    if (selectedJobOpening?.jobOpportunityId) {
      fetchPaginatedHistory(selectedJobOpening?.jobOpportunityId);
    }
  }, [dispatch, fetchPaginatedHistory, selectedJobOpening?.jobOpportunityId]);
  ...
  const customizedContent = (item: JobOpeningHistoryInterface) => {
  ...
  return (
    <div className="w-full px-5 pt-5 bg-white rounded-lg ">
      ...
      {(loading || jobOpeningLoading) && <AtsLoader />}
