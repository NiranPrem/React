...
import "./JobRequestOverview.css";
import type { RootState } from "../../../store/store";
import CloseLayout from "../../../shared/components/close-layout/CloseLayout";
import { fetchJobRequestByIdRequest } from "../../../store/reducers/jobRequestSlice";
...
const JobRequestOverview = () => {
  ...
  const { selectedJobRequest, loading } = useSelector(
  ...
  const tabComponents = useMemo(
    () => [<OverviewTab key="overview" />, <HistoryTab key="history" />],
    []
  );
  ...
  return (
    <div className="relative w-full text-[#181818] bg-white min-w-[1024px]">
      ...
      <CloseLayout
        title={selectedJobRequest?.positionTitle ?? t("common.myJobRequests")}
      />
