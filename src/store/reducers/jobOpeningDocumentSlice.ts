/* eslint-disable @typescript-eslint/no-unused-vars */
import { createSlice } from "@reduxjs/toolkit";
import type { PayloadAction } from "@reduxjs/toolkit";
import type { DocumentInterface } from "../../shared/interface/DocumentInterface";
import type { JobInterface } from "../../shared/interface/JobInterface";

// Define the structure of a document
interface Documents {
  data: DocumentInterface[];
  totalCount?: number;
}

// Define the structure of the documents state
interface DocumentsState {
  loading: boolean;
  error: string | null;
  totalCount?: number;
  documents: DocumentInterface[] | null;
  deleteSuccess: boolean;
  updateSuccess: boolean;
}

// Initial state for the job opening document slice
const initialState: DocumentsState = {
  loading: false,
  error: null,
  totalCount: 0,
  documents: null,
  deleteSuccess: false,
  updateSuccess: false,
};

// Create a slice for job opening documents with actions and reducers
const jobOpeningDocumentSlice = createSlice({
  name: "jobOpeningDocuments",
  initialState,
  reducers: {
    // Fetch documents requests
    fetchJobOpeningDocumentsRequest: (
      state,
      action: PayloadAction<{
        pageNumber: number;
        pageSize: number;
        jobOpportunityId: string;
      }>
    ) => {
      state.deleteSuccess = false;
      state.updateSuccess = false;
      state.loading = true;
      state.error = null;
    },
    // Fetch documents success
    fetchJobOpeningDocumentsSuccess: (
      state,
      action: PayloadAction<Documents>
    ) => {
      const { data, totalCount = 0 } = action.payload;
      state.documents = data;
      state.totalCount = totalCount;
      state.loading = false;
      state.error = null;
    },
    // Fetch documents failure
    fetchJobOpeningDocumentsFailure: (state, action: PayloadAction<string>) => {
      state.loading = false;
      state.error = action.payload;
      state.documents = [];
      state.totalCount = 0;
    },
    // Delete document request
    deleteJobOpeningDocumentRequest: (
      state,
      action: PayloadAction<{ documentFileId: number }>
    ) => {
      state.loading = true;
      state.error = null;
      state.deleteSuccess = false;
      state.updateSuccess = false;
    },
    // Delete document success
    deleteJobOpeningDocumentSuccess: (state, action: PayloadAction<number>) => {
      const id = action.payload;
      if (state.documents) {
        state.documents = state.documents.filter(
          (item) => item.documentFileId !== id
        );
      }
      state.totalCount = (state.totalCount ?? 0) - 1;
      state.loading = false;
      state.error = null;
      state.deleteSuccess = true;
    },
    // Delete document failure
    deleteJobOpeningDocumentFailure: (state, action: PayloadAction<string>) => {
      state.loading = false;
      state.error = action.payload;
      state.deleteSuccess = false;
    },
    // Update document
    updateJobOpeningDocumentRequest: (
      state,
      action: PayloadAction<JobInterface>
    ) => {
      state.loading = true;
      state.error = null;
      state.deleteSuccess = false;
      state.updateSuccess = false;
    },
    // Update document success
    updateJobOpeningDocumentSuccess: (
      state,
      action: PayloadAction<JobInterface>
    ) => {
      state.loading = false;
      state.error = null;
      state.updateSuccess = true;
    },
    // Update document failure
    updateJobOpeningDocumentFailure: (state, action: PayloadAction<string>) => {
      state.loading = false;
      state.error = action.payload;
      state.updateSuccess = false;
    },
  },
});

export const {
  fetchJobOpeningDocumentsRequest,
  fetchJobOpeningDocumentsSuccess,
  fetchJobOpeningDocumentsFailure,
  deleteJobOpeningDocumentRequest,
  deleteJobOpeningDocumentSuccess,
  deleteJobOpeningDocumentFailure,
  updateJobOpeningDocumentRequest,
  updateJobOpeningDocumentSuccess,
  updateJobOpeningDocumentFailure,
} = jobOpeningDocumentSlice.actions;

export default jobOpeningDocumentSlice.reducer;
